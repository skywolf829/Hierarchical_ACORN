...
def PSNRfromMSE(mse, max_diff = None):
    if(max_diff is None):
...
window_size : torch.Tensor, channel : int, size_average : Optional[bool] = True):
    ...
    if size_average:
        ...
        ans = mu1_mu2.mean(1).mean(1).mean(1)
    return ans
...
def bilinear_interpolate(im, x, y):
    ...
    x0 = torch.floor(x).clamp_(0, im.shape[3]-1).type(torch.long)
    x1 = (x0 + 1).clamp_(0, im.shape[2]-1)
    
    y0 = torch.floor(y).clamp_(0, im.shape[2]-1).type(torch.long)
    y1 = (y0 + 1).clamp_(0, im.shape[2]-1)
...
def local_to_global(local_coords, block_shapes, block_positions, full_shape):
    ...
    for d in range(2, len(full_shape)):
        global_positions[...,d-2] *= (block_shapes[...,d]/full_shape[d])
